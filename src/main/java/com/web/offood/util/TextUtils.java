...
public class TextUtils {
    ...
    private static String u = "ÚÙỤúùụƯỨỪỰưứừ" + "ựỦŨủũỬỮửữ";
    private static String y = "ÝỲỴýỳỵỶỸỷỹ";
    private static String d = "Đđ";
    public static final String EMPTY = "";
    public static final String PHONE_REGEX =
            "^(05|03|04|07|08|01[2689]|09|024|028)[0-9]{8}$|(021[012345689]|023[23456789]|020[3456789]|022[0123456789]|029[01234679]|025[123456789]|026[01239]|027[01234567])[0-9]{7}$";
    public static final String PASSWORD_REGEX = "^[^\\W_]{6,30}$";
    ...
    public static String hideNameWithDot(String name) {
        if (TextUtils.isEmpty(name)) return "";
        String names[] = name.trim().replaceAll("  ", " ").split(" ");
        StringBuilder strBuild = new StringBuilder();
        for (String str : names) {
            strBuild.append(str.replaceAll("\\b(.{2}).+(.{1})", "$1*****$2")).append(" ");
        }
        return strBuild.toString().trim();
    }

    public static String hideEmailWithDot(String email) {
        if (TextUtils.isEmpty(email)) return "";
        String emails[] = email.trim().split("@");
        if (emails.length != 2) {
            return email.replaceAll("\\b(.{3}).+(.{3})", "$1*****$2");
        }
        return emails[0].replaceAll("\\b(.{3}).+(.{3})", "$1*****$2") + "@" + emails[1];
    }
    ...
    public static String convertStringUTF8(String str) {
        ...
        if (!TextUtils.isEmpty(str)) {
            for (int i = 0; i < str.length(); i++) {
                if (Character.isLetter(str.charAt(i))) sb.append(convertCharUTF8(str.charAt(i)));
    ...
    public static String safeToString(Object obj1, String defaultValue) {
        ...
        String result = obj1.toString().trim();
    ...
    public static String getCorrectPhoneV2(String phone) {
        if (TextUtils.isEmpty(phone)) {
            return phone;
        }
        phone = phone.trim();
    ...
    public static String convertStringToNanoSpace(String showTypeName) {
        if (showTypeName == null) return "null";
        return convertStringUTF8(showTypeName).replaceAll("  ", " ").replaceAll(" ", "_").toUpperCase();
    }

    public static double roundDouble(double value, int num) {
        double pow = Math.pow(10, num);
        return ((double) Math.floor(value * pow)) / pow;
    }

    public static boolean validOrderNumber(String orderNumber) {
    ...
    public static boolean isChanged(Long s1, Long s2) {
        if (s1 == null && s2 != null) return true;
        if (s2 == null && s1 != null) return true;
        if (s1 == null && s2 == null) return false;
        if (s1.equals(s2)) return false;
    ...
    public static String genRefCode(Long userId) {
        StringBuilder refCode = new StringBuilder(Long.toHexString(userId));
        Random random = new Random();
        for (int i = refCode.length(); i < 8; i++) {
            refCode.insert(0, (char) (random.nextInt(20) + 'G'));
        }
    ...
    public static String genIcon(String heart, int numbers) {
    ...
    public static String formatString(String ob) {
        if (Objects.equals(ob, "")) return null;
        return ob;
    }
